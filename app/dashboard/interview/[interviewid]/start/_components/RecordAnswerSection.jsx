...
function RecordAnswerSection({ mockInterviewQuestion, activeQuestionIndex, interviewData }) {
  ...
  const [isRecording, setIsRecording] = useState(false); // added separate state
  ...
  useEffect(() => {
    if (!isRecording && userAnswer.length > 10) {
      // Do not call UpdateUserAnswer here, background queue handles it
    }
  }, [isRecording]);

  const StartStopRecording = () => {
    if (isRecording) {
      stopSpeechToText();          // Stop recording immediately
      setIsRecording(false);       // Reset button immediately
      queueAnswerProcessing();     // Background processing of answer
    } else {
      startSpeechToText();
  ...
  const queueAnswerProcessing = async () => {
